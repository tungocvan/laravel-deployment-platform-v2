...
# Legacy SSH aliases may no longer exist on the host. For GitHub-shaped SCP URLs,
# canonicalize only the transport used to read the source repository. Inventory and
# the site's origin remain untouched until the final verified cutover.
...
# A bootstrap target must be truly empty. Any branch, tag, or other advertised ref
# means the operator must clean/create another repository before retrying.
target_refs="$(git ls-remote "$target_repo" 2>/dev/null || true)"
if [[ -n "$target_refs" ]]; then
  echo "========================================================="
  echo "EMPTY REPOSITORY CHECK — FAILED"
  echo "========================================================="
  echo "New repo     : $target_repo"
  echo "State        : NOT EMPTY"
  echo "---------------------------------------------------------"
  printf '%s\n' "$target_refs" | head -20
  echo "========================================================="
...
trap 'git -C "$path" update-ref -d "$source_ref" >/dev/null 2>&1 || true' EXIT

source_tree="$(git -C "$path" rev-parse "$source_ref^{tree}")"
if ! git -C "$path" merge-base --is-ancestor "$site_head" "$source_ref" >/dev/null 2>&1; then
...
# Verify both write and delete permission without creating main yet.
probe_ref="refs/platform/write-probe/bootstrap-main"
if ! git -C "$path" push "$target_repo" "$source_ref:$probe_ref" >/dev/null; then
...
if ! git -C "$path" push "$target_repo" ":$probe_ref" >/dev/null; then
...
if ! git -C "$path" push "$target_repo" "$source_ref:refs/heads/main" >/dev/null; then
...
git -C "$path" fetch --no-tags "$target_repo" "refs/heads/main:$verify_ref" >/dev/null
...
if ! git -C "$path" fetch --prune origin >/dev/null; then
  rollback_remote
