...
site="${1:-}"
shift || true
[[ -n "$site" ]] || platform_die "$PLATFORM_EXIT_USAGE" "GIT.ARGUMENT_REQUIRED" "USAGE: platform-v2 git migrate-remote <site> --to=<empty-git-url> --bootstrap-empty-target [--dry-run] [--yes]"
...
set +e
target_refs="$(git ls-remote "$target_repo" 2>/dev/null)"
target_read_exit=$?
set -e
