...
site_ops_stale_empty_legacy_overlay() {
  ...
  case "$candidate" in compose.queue.yaml|compose.queue.yml|compose.scheduler.yaml|compose.scheduler.yml|compose.socket.yaml|compose.socket.yml) ;; *) return 1 ;; esac
  [[ -f "$path/$candidate" ]] || return 1
  absolute="$(readlink -f "$path/$candidate" 2>/dev/null || true)"; [[ -n "$absolute" ]] || return 1
...
site_ops_dirty_report() {
  ...
  status="$(git -C "$path" status --porcelain --untracked-files=all)"; [[ -n "$status" ]] || return 1
  tracked="$(printf '%s\n' "$status" | awk 'substr($0,1,2) != "??" {print substr($0,4)}')"
  untracked="$(printf '%s\n' "$status" | awk 'substr($0,1,2) == "??" {print substr($0,4)}')"
  runtime_files="$(site_ops_runtime_config_files "$site" "$path")"
  while IFS= read -r file; do
    [[ -n "$file" ]] || continue
    if site_ops_runtime_owned_overlay "$path" "$file" "$runtime_files"; then managed+="${managed:+$'\n'}$file"
    elif site_ops_stale_empty_legacy_overlay "$path" "$file" "$runtime_files"; then stale+="${stale:+$'\n'}$file"
    else blocking+="${blocking:+$'\n'}$file"; fi
  done <<<"$untracked"
  if [[ -n "$managed" ]]; then echo "ACTIVE RUNTIME-MANAGED OVERLAYS (verified from live Docker Compose labels):"; printf '%s\n' "$managed" | sed 's/^/  - /'; echo "[OK] Các overlay active này được giữ nguyên và không làm BLOCK Update Site."; fi
  if [[ -n "$stale" ]]; then echo "STALE EMPTY PLATFORM OVERLAYS (legacy placeholders; not active runtime):"; printf '%s\n' "$stale" | sed 's/^/  - /'; echo "[WARNING] Các file rỗng legacy này không làm BLOCK Update Site và KHÔNG bị tự động xóa."; echo "          Có thể dọn riêng bằng Production Cleanup sau khi preview/confirm."; fi
  [[ -z "$tracked" && -z "$blocking" ]] && return 1
  echo "========================================================="; echo "UPDATE SITE — BLOCKED: WORKING TREE NOT CLEAN"; echo "========================================================="
  ...
  echo "Không tự git add, git clean hoặc xóa các file blocking trên."; echo "Update bị BLOCK để bảo vệ local changes hoặc runtime overlays chưa xác minh."; return 0
}
...
site_ops_update() {
  ...
  echo "Site          : $site"; echo "Branch        : $branch"; echo "Current       : $before"; echo "Upstream      : $after"; echo "Build needed  : $build"; echo "Migration risk: $migration_risk"; echo "Run migration : $migrate"; echo "Changed files :"
  ...
  if [[ "$migration_risk" -eq 1 && "$migrate" -eq 0 ]]; then
    if [[ "$dry" -eq 1 ]]; then echo "[MIGRATION-REQUIRED] Preview phát hiện migration mới. UI/operator phải xác nhận --migrate trước khi apply."; return 3; fi
    echo "[BLOCKED] Có migration mới. Chỉ chạy khi operator chủ động thêm --migrate."; return 3
  fi
...
site_ops_env_apply() {
  ...
  local app; app="$(site_runtime_app_service "$path")"; deploy_compose "$path" exec -T "$app" php artisan config:clear; deploy_compose "$path" exec -T "$app" php artisan config:cache; deploy_compose "$path" exec -T "$app" php artisan queue:restart >/dev/null 2>&1 || true
